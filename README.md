...
* `Leon`, a `Farmer` has a `Farm` consisting of
	...
	* 2 `FarmVehicle`
	* 1 `Aircraft`
	* His sister, `Leona`, is a `Pilot` who uses a `CropDuster`.



# The Plot Thickens...
* Create test cases which simulate a typical work-week for `Leon` and `Leona`.
...
* Create a separate Test class for each day in the following scenario.
	* As a farmer, Froilan must stick to a rigorous agricultural routine.
		* Every morning, `Leon` and `Leona` begin their morning by
			* Riding each `Horse` in each `Stable`.
			* Feeding each `Horse` 3 ear of `Corn`.
			* For breakfast,
				* `Leon` eats 1 `EarCorn`, 2 `Tomoato`, and 5 `Egg`.
				* `Leona` eats 2 `EarCorn`, 1 `Tomoato`, and 2 `Egg`.

		* `Sunday`, `Leon` plants 3 different type of `Crop` in his first, second, and third `CropRow`.
		* On `Monday`, his sister, `Leona` uses the `CropDuster` to `fly` over the `Field` and `fertilize` each of the `CropRow`
		* On `Tuesday`, `Leon` uses his `Tractor` to `harvest` each `Crop` in each `CropRow`.
